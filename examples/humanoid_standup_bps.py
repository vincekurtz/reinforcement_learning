...
"""
Use Boltzmann Policy Search to train a humanoid standup task.
"""
...
def train():
    ...
    policy_net = MLP(layer_sizes=(32, 32, 17))
    ...
    bps = BoltzmannPolicySearch(
        ...
        policy=policy_net,
        options=options,
        tensorboard_logdir="/tmp/rl_playground/humanoid_bps",
    )
    params = bps.train(iterations=2000, num_evals=10)

    fname = "/tmp/humanoid_bps.pkl"
...
def test():
    ...
    with open("/tmp/humanoid_bps.pkl", "rb") as f:
        data = pickle.load(f)
...
if __name__ == "__main__":
    usage_message = "Usage: python humanoid_standup_bps.py [train|test]"
